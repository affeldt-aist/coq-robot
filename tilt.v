...
Lemma LieDerivative_norm {K : realType} n (f : 'rV[K]_n.+1 -> 'rV_n.+1)
    (phi : 'rV[K]_n.+1 -> K -> 'rV_n.+1)
    (x : 'rV[K]_n.+1) (t : K) :
     differentiable f (phi x t) -> 
       differentiable (phi x) t ->
     ...
     (f (phi x t) != 0) ->
    LieDerivative (fun y => `|f y|_e ^+ 2) phi x t =
    (2%:R *: 'D_1 (f \o phi x) t *m (f (phi x t))^T) 0 0.
Proof.
move => diffp difpx difft0 neq0.
rewrite LieDerivative_derive => //=; last first.
  apply: differentiable_enorm_squared => //=.
rewrite -derive1E /=.
rewrite fctE.
replace (fun x0 : K => `|f (phi x x0)|_e ^+ 2)
  with ((1 \*o (GRing.exp (R:=K))^~ 2 \o enorm) \o (f \o phi x)); last first.
  rewrite !fctE.
  rewrite -fctE.
  apply/funext => s.
  by rewrite /= /GRing.exp mul1r. 
rewrite derive_enorm_squared => //=; last first.
  apply: diff_derivable=> //=.
  apply: differentiable_comp => //=.
...
Theorem Lyapunov_stability {K : realType} {n}
  ...
  (D : set 'rV[K]_n.+1)
  (V : 'rV[K]_n.+1 -> K)
  (fdtraj : forall x0, x0 \in D -> solves_equation f (sol x0) D)
  (x : 'rV[K]_n.+1 := 0)
  (Vx0 : is_lyapunov_candidate V D x) (*contient l'hypothese x in D*)
  (t : K)
  (V'le_0 : forall s t, s \in D -> LieDerivative V sol s t  <= 0) :
...
have HOmega_beta : Omega_beta `<=` interior (closed_ball_ [eta normr])``_r.
  ...
  case: (ltgtP (`|x1|) r) => [Hlt | Heq | Hgt].
    ...
    by exact: r_pos.
have H1 : forall t, sol x 0 \in Omega_beta -> t >= 0 -> sol x t \in Omega_beta.
  move => t0 xOmega t00 .
  have H2 : forall x t, x \in Omega_beta -> t >= 0 -> LieDerivative V sol x t <= 0  -> V (sol x t) <= V (sol x 0) <= beta.
    move => x1 t1 x1Omega t10 Lie0. 
    ...
    (* LEMMA*)
      ...
      - move: Hs1_pos => /andP [H0s1 Hs1s2].
      by apply: Hs1s2 => //=.
    have H3 : V (sol x1 t1) <= V (sol x1 0).
      apply: Vneg_incr => //=.
        apply/andP; split => //=.
        rewrite inE.
        apply: Br_sub_D.
    ...
    by [].
  rewrite inE.
  have V_bound := H2 x t0 _ t00 (V'le_0  _ t0 _).
  split; last first => //=.
    have x0_in_D : sol x 0 \in D.
      rewrite inE.
      apply: (Br_sub_D).
      move: xOmega.
      rewrite inE.
      move => [Hball _].
      exact: Hball.
    rewrite init in xOmega.
    have Vchain := V_bound xOmega inD.
    have /andP [Vt0_le_V0 V0_le_beta] := Vchain.
    exact: le_trans Vt0_le_V0 V0_le_beta.
    rewrite /closed_ball_; under eq_fun do rewrite sub0r normrN.
  have Hx0: (sol x 0) \in closed_ball_ [eta normr] 0 r.
    rewrite inE.
    case: xOmega.
    rewrite inE.
    case.
...
Theorem Lyapunov_stability {K : realType} {n}
  ...
  (x0inD : x0 \in D)
  (V : 'rV[K]_n.+1 -> K)
  (fdtraj : solves_equation f sol D) (* continuously diff*)
  (sol0 : sol 0 \in D)
  (Vx0 : is_lyapunov_candidate V x0)
  (V'le_0 : LieDerivative V sol 0 <= 0):
...
Theorem Lyapunov_asymptotic_stability {K : realType} {n}
  ...
  (forall sol : (K -> 'rV[K]_n.+1),
      solves_equation f sol A ->
      locnegsemidef (LieDerivative V sol) 0 ->
       sol 0 = x0 ) ->
...
Definition state_space_tilt {K : realType} := [set x : 'rV[K]_6 | `|'e_2 - Right x|_e = 1].
...
- move=> p.
  ...
  split.
  split.
  by rewrite inE /= rsubmx_const subr0 enormeE.
  split.
  apply: derivable_cst => //.
  move => t.
...
Lemma V1_is_lyapunov_candidate A : open A -> point1 \in A -> is_lyapunov_candidate V1 A point1.
Proof.
move=> openA point1A.
split.
...
(*    simpl in *.
  ...
  rewrite /V1.
  have /orP[lz0|rz0] : (Left z != 0) || (Right z != 0).
  + rewrite -negb_and.
...
Lemma norm_u1 (traj : K -> 'rV_6) (z : K) (z2 := Right \o traj)
  ...
  solves_equation (tilt_eqn alpha1 gamma) traj state_space_tilt -> `|u|_e = 1.
Proof.
move=> dtraj.
...
rewrite LieDerivativeMl; last first.
  ...
  exact (fun z => gamma *: ((Right z - Left z) *m \S('e_2 - Right z) ^+ 2)).
    apply/differentiable_rsubmx => //.
rewrite !fctE !LieDerivative_enorm_squared /=; last 6 first.
  apply/differentiable_rsubmx => //.
  apply (dif1 t).
  move => t0.
  apply/differentiable_rsubmx => //.
...
(*  under eq_bigr do rewrite mulNmx. FIXME *)
  admit.
  by apply/ltW => //.
  replace z0 with z.
  admit.
