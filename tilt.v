...
rewrite -derivemxE; last first.
  apply/differentiable_comp; last first.
    exact: differentiable_scalar_mx.
  rewrite -fctE /=.
  apply: differentiableM; apply/differentiable_enorm => //=.
have := derive_norm.
rewrite //=.
...
rewrite derive1Ml; last 1 first.
  ...
  apply: (@differentiable_comp _ _ _ _ x (enorm \o f)) => //;
  by apply: differentiable_enorm.
rewrite fctE /=.
rewrite mul1r.
rewrite !mxE.
rewrite derive1E.
...
rewrite -diff_comp //=.
rewrite -fctE /=.
by apply: differentiableM; by apply: differentiable_enorm.
Qed.

Lemma LieDerivative_enorm_squared {K : realType} (f : 'rV[K]_6 -> 'rV_3)
  (x : K -> 'rV[K]_6) (t : K) :
     differentiable f (x t) ->
    differentiable x t -> (forall t, differentiable f t) ->
    LieDerivative (fun y => `|f y|_e ^+ 2) x t =
...
Theorem Lyapunov_stability {K : realType} {n}
  ...
  (traj10 : traj1 0 \in D)
  (Vx0 : is_lyapunov_candidate V D x0)
...
Theorem Lyapunov_asymptotic_stability {K : realType} {n}
  ...
  (V : 'rV[K]_n.+1 -> K)
  (x0 : 'rV[K]_n.+1) :
  (is_equilibrium_point f x0 A /\ is_lyapunov_candidate V A x0) ->
