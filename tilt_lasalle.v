...
apply: (locally_cauchy_lipschitz_unique _ _ hs) => //.
  ...
  apply /derivable1_diffP.
  apply is_sol => //.
  by move : t0t; rewrite inE/=in_itv/= => /andP[].
move => t0 t00 t0t.
...
have : lasalle.is_sol phi (sol p) by apply sol_spec; rewrite ?initp.
move=> [/= _ H] t t0.
split.
...
split => //.
  ...
  suff h: 0 < t.
    ...
    move : t0p.
    by rewrite closure_neitv_oo// inE/= in_itv/= => /andP[].
    rewrite ltNge.
    move : t0p.
    apply contraPN => h0.
...
- move=> t' itvt'.
  apply isSol => //.
    by rewrite inE.
    by move: itvt'; rewrite !in_itv /= => /andP[t'0 _]; rewrite ltW.
- apply: continuous_in_subspaceT.
  move => t' t'p.
  apply: differentiable_continuous.
  apply /derivable1_diffP.
...
suff : forall t, 0 < t ->  {in sublevelUpsilon1 p, continuous (from_subspace (sublevelUpsilon1 p) (sol^~ t))}.
...
apply/cvgrPdist_le => //= e e0.
  ...
  have [r' pur] : exists r' : {posnum K}, sublevelUpsilon1 p `<=` closed_ball u r'%:num.
    ...
    rewrite -addrA addrC -addrA lerD //.
    apply Mb => //.
  ...
  near=>v.
  move => pv.
  have := @thm34 K 6 (fun=> phi) (cst 0) 0 (t + 1) t01 k'%:num u v r' k'0 k'r'phi.
